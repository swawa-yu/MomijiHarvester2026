...
def validate_target_year(
    ...
    academic_year = (
        manifest.get("academicYear") if isinstance(manifest, dict) else None
    )
    match = re.fullmatch(r"(\d{4})年度", academic_year or "")
    if not match:
        raise ValueError(
            "consumer manifest academicYear must have the form YYYY年度"
        )
    ...
    if target != expected:
        raise ValueError(
            f"{update_kind} requires target_year={expected}, "
            f"consumer active year is {active_year}"
        )
